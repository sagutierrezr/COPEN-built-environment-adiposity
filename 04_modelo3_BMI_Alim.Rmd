...
#----------------------------
# 3) Componentes para IC Wald del slope ciudad-específico
...
sd_city_food <- BASE_ESTUDIO_BMI_Z %>%
  group_by(CITY_NAME) %>%
  summarise(
...
```{r}
#=========================================================
# MOD3_F1: Tabla por ciudad (Wald) para:
#  - DENS_HEALTH_LOG_ZC por SEXO y por NIVEL EDU (bloques separados)
#  - DENS_UNHEALTH_LOG_ZC por NIVEL EDU
#  - + Promedio global (u=0)
# Usa: mod$coef, mod$cov_mat, mod$varVC, mod$ranefMat
#=========================================================
...
stopifnot(all(c("DENS_HEALTH_LOG_ZC","DENS_UNHEALTH_LOG_ZC") %in% names(u_city)))
...
#----------------------------
# 3) Nombres de exposiciones
#----------------------------
b_H <- "DENS_HEALTH_LOG_ZC"
b_U <- "DENS_UNHEALTH_LOG_ZC"

stopifnot(all(c(b_H, b_U) %in% names(mod$coef)))

# Interacciones con SEX
term_H_sex <- function(sex_level){
  if (sex_level == sex_ref) return(NA_character_)
  get_term(b_H, paste0("SEX", sex_level))
}

# Interacciones con EDU
term_H_edu <- function(edu_level){
  if (edu_level == edu_ref) return(NA_character_)
  get_term(b_H, paste0("EDU_GROUP", edu_level))
}
term_U_edu <- function(edu_level){
  if (edu_level == edu_ref) return(NA_character_)
  get_term(b_U, paste0("EDU_GROUP", edu_level))
}

cities <- u_city$CITY_NAME

#=========================================================
# A) DENS_HEALTH_LOG_ZC por SEXO (sin EDU)
#=========================================================
df_H_sex_city <- crossing(
  CITY_NAME = cities,
  SEX = sex_levels
) %>%
  rowwise() %>%
  mutate(
    ...
    u_h = u_city[[b_H]][match(CITY_NAME, u_city$CITY_NAME)],
    est0 = beta_fix + u_h,
  ...
  ungroup() %>%
  bind_cols(wald(.$est0, .$var0)) %>%
  transmute(
    CITY_NAME,
    Atributo  = "Densidad de oferta saludable (log; Z por ciudad)",
    Grupo     = "Sexo",
...
pop_H_sex <- crossing(
  ...
  mutate(
    terms = list(c(b_H, term_H_sex(SEX))),
    est0 = sum(mod$coef[unlist(terms)[!is.na(unlist(terms))]]),
    var0 = vcov_lincomb(unlist(terms))
  ) %>%
  ungroup() %>%
  bind_cols(wald(.$est0, .$var0)) %>%
  transmute(
...
#=========================================================
# B) DENS_HEALTH_LOG_ZC por NIVEL EDU (sin SEX)
#=========================================================
df_H_edu_city <- crossing(
  CITY_NAME = cities,
...
#=========================================================
# C) DENS_UNHEALTH_LOG_ZC por NIVEL EDU
#=========================================================
df_U_edu_city <- crossing(
  CITY_NAME = cities,
  EDU_GROUP = edu_levels
) %>%
  rowwise() %>%
  mutate(
    ...
    beta_fix = sum(mod$coef[unlist(terms)[!is.na(unlist(terms))]]),
    u_u = u_city[[b_U]][match(CITY_NAME, u_city$CITY_NAME)],
    est0 = beta_fix + u_u,
    var_fix = vcov_lincomb(unlist(terms)),
  ...
  transmute(
    CITY_NAME,
    Atributo  = "Densidad de oferta no saludable (log; Z por ciudad)",
    Grupo     = "Nivel educativo",
    Estrato   = as.character(EDU_GROUP),
    Contraste = "por 1 SD (Z por ciudad)",
    est, lwr, upr
...
tabla_BMI_food <- bind_rows(
  df_H_sex_city, pop_H_sex,
  df_H_edu_city, pop_H_edu,
  df_U_edu_city, pop_U_edu
) %>%
  mutate(
    ...
    Atributo = factor(Atributo, levels = c(
      "Densidad de oferta saludable (log; Z por ciudad)",
      "Densidad de oferta no saludable (log; Z por ciudad)"
    )),
    Grupo = factor(Grupo, levels = c("Sexo","Nivel educativo")),
    Estrato = case_when(
      Grupo == "Nivel educativo" ~ factor(Estrato, levels = levels(dat$EDU_GROUP)),
      Grupo == "Sexo"            ~ factor(Estrato, levels = levels(dat$SEX)),
      TRUE ~ factor(Estrato)
    ),
...
```{r}
#============================
# Forest plot SUPERMERCADOS (DENS_HEALTH_LOG_ZC) por ciudad
# 1 fila por ciudad, 2 sexos apilados
# Orden: BOGOTA, MEDELLIN, CALI, BARRANQUILLA, BUCARAMANGA
# Promedio poblacional al final + línea punteada en 0
# Modelo: MOD3_F1
#============================
...
mod <- MOD3_F1
city_var <- "CITY_NAME"
expo <- "DENS_HEALTH_LOG_ZC"   # <- supermercados (según tu modelo)

#----------------------------
# 1) Términos (maneja ambas convenciones de interacción)
#----------------------------
b_EXPO <- expo
...
u_city <- u_city %>%
  rownames_to_column(var = "CITY_NAME")   # BOGOTA, MEDELLIN, etc.

#----------------------------
# 3) Componentes para IC (Wald aprox.)
#    Incluye var_u(expo) del nivel ciudad (como en tu script base)
#----------------------------
...
G <- mod$varVC[[city_var]]
stopifnot(all(c(expo, expo) %in% rownames(G)))
var_u_EXPO <- G[expo, expo]

#----------------------------
# 4) Efectos por ciudad y sexo
#    Hombre (ref): beta_EXPO + u_EXPO
#    Mujer:        beta_EXPO + beta_EXPOxS + u_EXPO
#----------------------------
df_city <- u_city %>%
  transmute(
    CITY_NAME = CITY_NAME,
...
pop_m <- tibble(
  ...
  lwr   = (beta_EXPO + beta_EXPOxS) - 1.96*sqrt(var_beta_EXPO + var_beta_EXPOxS + 2*cov_EXPO_EXPOxS),
  upr   = (beta_EXPO + beta_EXPOxS) + 1.96*sqrt(var_beta_EXPO + var_beta_EXPOxS + 2*cov_EXPO_EXPOxS)
)

#----------------------------
# 6) Orden requerido + apilado por sexo en 1 fila
#----------------------------
city_order <- c("BOGOTA","MEDELLIN","CALI","BARRANQUILLA","BUCARAMANGA","Promedio poblacional")
...
#----------------------------
# 7) Plot (misma estética que tu script)
#----------------------------
...
ggplot(plot_df, aes(x = slope, y = y, color = sex)) +
  ...
  # (tu "tick" en negrita; aquí lo dejo igual aunque el label esté vacío)
  annotate("text",
           x = x_label, y = y_prom,
           label = "",
...
```{r}
#=========================================================
# Forest plot: efecto de SUPERMERCADOS (DENS_HEALTH_LOG_ZC) sobre IMC por ciudad,
...
# Si el modelo guardó la interacción al revés (expo:EDU), corrige automáticamente
...
#----------------------------
# 7) Plot (misma estética + tu paleta fija)
#----------------------------
pal_edu <- c(
  "Primaria" = "#D55E00",
  "Secundaria o técnico" = "#009E73",
...
```{r}
#=========================================================
# Forest plot: efecto de RESTAURANTES (DENS_UNHEALTH_LOG_ZC) sobre IMC por ciudad,
# estratificado por EDU_GROUP (apilado por nivel educativo)
# Promedio poblacional al final + franja + separador + línea en 0
# Modelo: MOD3_F1 | Datos: BASE_ESTUDIO_BMI_Z
#=========================================================
...
ggplot(plot_df, aes(x = slope, y = y, color = CITY_NAME)) +
  ...
  scale_y_reverse(
    breaks = seq_along(city_order),
    labels = c("Bogotá","Medellín","Cali","Barranquilla",
