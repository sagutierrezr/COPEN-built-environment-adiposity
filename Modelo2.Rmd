...
waldTest(m, type = "beta", coefs = c("EDU_GROUPSecundaria o técnico:STOPS_Q4_NUM",
...
diag_quad <- get_RME(m2_quad, dat) %>% mutate(model = "Cuadrático")
...
#========================
# 6) (Opcional) AIC/BIC para complementar la decisión
#========================
...
ic_quad <- get_ic_wemix(m2_quad, n = nrow(dat)) %>% mutate(model="Cuadrático")
...
#    (A) Según tu flujograma: RCE vs Edad
#========================
...
#    (ajusta los objetos a tus nombres reales)
#    - m_L2_lin
#    - m_GREEN_quad
#    - m_GREEN_cubic
#========================================================
...
#========================================================
# 4) Pasar a formato largo SOLO para el intercepto (más limpio)
#    Nota: en ranefMat el intercepto suele llamarse "(Intercept)"
#========================================================
...
Aunque el LR mejoró, gráficamente la inclusión de un término cúbico no mejoró notablemente el ajuste de la linealidad.
...
#===========================================================
# 1) Parámetros de interés (fijos y varianzas)
#===========================================================
...
#===========================================================
# 4) Pre-extracción del modelo FULL (optimiza)
#===========================================================
...
#----------------------------
# 5) Construir tabla en formato “como tu imagen”
#----------------------------
tab <- tibble(
  Variable = c(
...
#    Var(beta_city) ≈ Var(beta_fix) + Var(u_slope)  (misma aproximación que vienes usando)
#----------------------------
V <- mod$cov_mat
...
# (Opcional) STOPS: Q4 vs Q1 (Δ=3). Descomenta si lo quieres en la tabla:
...
#      (si quieres, puedo poner SD global o promedio ponderado de SDs)
#----------------------------
pop <- tibble(
...
#
# Var(Wald) usada (misma aproximación que vienes usando):
#  Var(beta_ciudad) ≈ Var(combinación fija) + Var(u_slope)
#=========================================================
...
# Global INT_Z por educación (u=0)
...
# Global GREEN_Z por educación (u=0)
...
#=========================================================
# C) STOPS_Q4_NUM (sin interacción) por ciudad
#=========================================================
beta_STOPS <- unname(mod$coef[b_STOPS])
...
tabla_WC <- bind_rows(
  ...
  mutate(
    ...
    # 🔹 Orden correcto de nivel educativo
...
# --- Paleta publicable (Okabe–Ito) + promedio en negro/gris
pal_city <- c(
  "BOGOTA" = "#0072B2",
  "MEDELLIN" = "#D55E00",
  "CALI" = "#009E73",
...
#----------------------------
# 1) Términos del modelo (según summary(MOD2_F1))
#----------------------------
b_INT <- "INT_Z"

edu_levels <- c("Primaria", "Secundaria o técnico", "Profesional o mayor")

# ✅ Nombres EXACTOS en MOD2_F1:
b_INTxEDU <- c(
...
#    (ojo: tu modelo tiene 5 ciudades)
#----------------------------
city_order <- c("BOGOTA","MEDELLIN","CALI","BARRANQUILLA","BUCARAMANGA","Promedio poblacional")
...
plot_df <- bind_rows(df_city, pop_df) %>%
  ...
  # Mantener solo las que están en tu orden (y evitar NA)
...
# Estética tipo “paper” (sin ggtext)
...
# Varianza del random slope en CITY_NAME (si por alguna razón no existiera -> 0)
...
ggplot(plot_df, aes(x = slope, y = CITY_NAME, color = city_label)) +
  ...
  labs(
    ...
    subtitle = "IC Wald aproximados; pendiente aleatoria por ciudad; sin estratificación."
  ) +
...
# deltas por educación (ref = Primaria)
delta_map <- tibble(
  EDU_GROUP = factor(edu_levels, levels = edu_levels),
